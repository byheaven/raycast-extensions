...
- `Favorite Track` and `Favorite and Add to Library` commands now close Raycast immediately and show HUD only after Apple Music confirms the change.
- Menu bar favorite status is now fetched in the same AppleScript call as the track, eliminating the flicker when opening the dropdown.
- Added scrolling marquee for long track names inside the menu bar dropdown.
- Moved the "Open Music" item to the top of the menu bar dropdown.
